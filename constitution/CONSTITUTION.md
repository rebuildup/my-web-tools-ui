# my-web-tools-ui Constitution

この文書は、my-web-tools-ui の開発組織における最上位 contract です。
...
my-web-tools-ui の開発組織の目的は current procedure への服従ではありません。
...
This repository owns shared UI primitives consumed by my-web-2025 tool packages. Consumer-provided hook implementations and standalone type-check stubs are distinct identities and must not be conflated.

## Upstream source

Governance baseline: `rebuildup/project-init@release-0-3-0`. Managed Agent Skills track current `rebuildup/project-init` through `bunx skills`.

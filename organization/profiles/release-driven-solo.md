...
my-web-tools-ui の current release-driven development で使用する Operating Model を定義します。
...
- my-web-2025 host work stateをこのrepositoryへcanonical copyしない
...
- `packageManager: bun@1.3.10` is the Bun version authority; mise uses the same version.
- shared UI source in this repo is canonical; consumer hook implementations remain consumer-owned.
- standalone stub hooks are type-check shims, not runtime behavior evidence.
- standalone lint/type-check and my-web-2025 integration/browser evidence are distinct gates.
...
- `skills-lock.json` records current project-init source/path/content hashes.
